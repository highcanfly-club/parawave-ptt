...
class PTTButton: UIButton {
  ...
  private func setupButton() {
    ...
    addTarget(
      self, action: #selector(buttonReleased), for: [.touchUpInside, .touchUpOutside, .touchCancel])
  }
...
class VolumeButtonHandler: NSObject {
  ...
  @objc private func volumeChanged(notification: NSNotification) {
    guard let userInfo = notification.userInfo,
      let volumeChangeType = userInfo[
        "AVSystemController_AudioVolumeChangeReasonNotificationParameter"] as? String
    else {
      return
    }
...
// MARK: - Debug Extensions
  ...
  extension MainViewController {
    ...
    func debugAddTestButtons() {
      ...
      NSLayoutConstraint.activate([
        testStackView.topAnchor.constraint(
