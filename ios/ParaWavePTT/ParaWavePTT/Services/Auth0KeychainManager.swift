...
class Auth0KeychainManager {
    ...
    func storeTokens(accessToken: String, refreshToken: String?, expiresIn: TimeInterval) throws {
        ...
        try storeInKeychain(
            key: expiryDateKey, value: ISO8601DateFormatter().string(from: expiryDate))
    ...
    /// Clear expired or invalid authentication tokens
    func clearExpiredTokens() throws {
        let tokenKeys = [accessTokenKey, refreshTokenKey, expiryDateKey]
        ...
        print("Expired tokens cleared from keychain")
    }
...
extension Notification.Name {
    ...
    static let auth0AuthenticationStateChanged = Notification.Name(
        "auth0AuthenticationStateChanged")
}
