...
class ParapenteNetworkService: NSObject, ObservableObject {
    ...
    func getChannels(type: ChannelType? = nil, 
        ...
        if let location = location {
            endpoint += "&lat=\(location.coordinate.latitude)&lon=\(location.coordinate.longitude)&radius=\(radius)"
        }
    ...
    func createChannel(_ channelRequest: CreateChannelRequest) async throws -> PTTChannel {
        ...
        guard let channel = response.data else {
            throw ParapenteError.networkError(NSError(domain: "CreateChannelFailed", code: 0, userInfo: nil))
        }
    ...
    func updateChannel(_ uuid: String, request: UpdateChannelRequest) async throws -> PTTChannel {
        ...
        guard let channel = response.data else {
            throw ParapenteError.networkError(NSError(domain: "UpdateChannelFailed", code: 0, userInfo: nil))
        }
    ...
    func getChannelParticipants(_ uuid: String) async throws -> [ChannelParticipant] {
        ...
        guard let participants = response.data else {
            throw ParapenteError.networkError(NSError(domain: "NoParticipants", code: 0, userInfo: nil))
        }
    ...
    private func performRequest<T: Codable>(_ request: URLRequest) async throws -> T {
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                throw ParapenteError.networkError(NSError(domain: "InvalidResponse", code: 0, userInfo: nil))
            }
            ...
            default:
                ...
                if let errorResponse = try? JSONDecoder.parawaveDecoder.decode(ErrorResponse.self, from: data) {
                    throw ParapenteError.networkError(NSError(domain: "APIError", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: errorResponse.error]))
                } else {
                    throw ParapenteError.networkError(NSError(domain: "HTTPError", code: httpResponse.statusCode, userInfo: nil))
                }
            }
...
struct NetworkInfo {
    ...
    var displayString: String {
        ...
        let typeString = connectionType == .cellular ? "Cellulaire" : connectionType == .wifi ? "WiFi" : "Autre"
